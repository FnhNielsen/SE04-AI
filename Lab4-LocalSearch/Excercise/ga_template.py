...
def reproduce(mother, father):
    ...
    '''
    child = []
    crossover = random.randint(1, len(mother))
...
def mutate(individual):
    ...
    '''
    if len(individual) > 0:
        random_num = random.randint(0, len(individual) - 1)
...
def random_selection(population, fitness_fn):
    ...
    for individual in ordered_population:
        fitness_list.append(fitness_fn(individual))

    father_index = fitness_list.index(max(fitness_list))
    mother_index = 0

    fitness_list.remove(max(fitness_list))
    if rand_value > p_mutation * 100:
        mother_index = fitness_list.index(max(fitness_list))
    else:
